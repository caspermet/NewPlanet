...
float EdgeTess(float3 wpos0, float3 wpos1, float edgeLen)
{
	// distance to edge center
	float dist = distance(0.5 * (wpos0 + wpos1), _CameraPosition);
	// length of the edge
...
HS_CONSTANT_OUTPUT HSConst(InputPatch<VS_OUTPUT, 3> v)
{
	...
	tf = edgee(vi[0].vertex, vi[1].vertex, vi[2].vertex, 5);
